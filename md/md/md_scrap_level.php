...
//-->
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td align="right">
        	<a onClick="document.location='scrap_level_input.php?mode=insert';" class="AW-btn">등급추가</a>
        </td>
      </tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="t_th">
        ...
        <th width="15%">등급레벨</th>
        <th width="15%">구매할인액(%)</th>
        <th width="15%">배송할인액(%)</th>
        <th width="10%">기능</th>
      </tr>
      <tr><td class="t_rd" colspan="20"></td></tr>
			<?
		 	$sql = "select * from wiz_scraplevel order by level asc, idx asc";
		 	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
		 	$total = mysqli_num_rows($result);
		 	$no = $total;
			while($row = mysqli_fetch_object($result)){
		
				if($row->buyFeeType == "W") $row->buyFeeType = "원";
				else  $row->buyFeeType = "% ";

        if($row->delFeeType == "W") $row->delFeeType = "원";
				else  $row->delFeeType = "% ";
			?>
      <tr class="t_tr"> 
        ...
        <td align="center"><?=$row->buyFeeAmt?><?=$row->buyFeeType?></td>
        <td align="center"><?=$row->delFeeAmt?><?=$row->delFeeType?></td>
        <td align="center">
        <?
        if($row->level != 0){
        ?>
        	<a onclick="document.location='scrap_level_input.php?mode=update&idx=<?=$row->idx?>';" class="AW-btn-s modify">수정</a>
