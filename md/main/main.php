    ...
    <div class="cont">
    	<div class="latest">
        	<table width="100%" border="0" cellpadding="0" cellspacing="0">
                ...
                <tbody>
					<?
					  $sql = "select wo.* from wiz_order as wo inner join wiz_basket as wb on wo.orderid = wb.orderid where wo.status != '' and wb.mallid = '".$wiz_md['id']."' order by wo.orderid desc limit 10";
					  ...
					  $total = mysqli_num_rows($result);
						  while($row = mysqli_fetch_array($result)){
							...
							unset($mall_list);
									...
									// 각 입점업체별 상품 수 구하기
									$b_sql = "select count(idx) as cnt, sum(prdprice) as total, mallid from wiz_basket where orderid='".$row['orderid']."' and mallid = '".$wiz_md['id']."' group by mallid";
									...
									$b_sql = "select wb.*, wm.com_name, wm.com_tel, wm.del_method, wm.del_fixprice, wm.del_staprice, wm.del_staprice2, wm.del_staprice3, wm.del_prd, wm.del_prd2
													...
													where wb.orderid='".$row['orderid']."' and wb.mallid = '".$wiz_md['id']."' order by wb.mallid";
									$b_result = mysqli_query($connect, $b_sql) or error(mysqli_error($connect));
...
<div class="manage-M-bbs">
    ...
    <div class="cont">
    	<ul class="latest">
			<?
			$limit = 8;
			$sql = "select *, date_format(from_unixtime(wdate), '%Y/%m/%d') as wdate, wdate as wtime from wiz_bbs where code != 'memout' and (code like 'mall_%' or code = 'qna') and mallid = '".$wiz_md['id']."' order by idx desc limit $limit";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
			$total = mysqli_num_rows($result);
    ...
    <div class="cont">
    	<ul class="latest">
			...
			$sql = "select wb.idx, wb.name, wb.prdcode, wb.subject, date_format(from_unixtime(wb.wdate), '%Y/%m/%d') as wdate, wb.wdate as wtime
							...
							where wb.code = 'review' and wb.prdcode != '' and wp.mallid = '".$wiz_md['id']."'
