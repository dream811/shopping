...
$sql = "select distinct wp.prdcode from wiz_product wp, wiz_cprelation wc
              where $catcode_sql $special_sql $showset_sql $search_sql $coupon_sql $brand_sql $shortage_sql $stock_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr><td class="t_rd" colspan="20"></td></tr>
	...
	$sql = "select distinct wp.prdcode, wp.prdimg_R, wp.prdname, wp.sellprice, wp.prior, wp.stock from wiz_product wp, wiz_cprelation wc
	            where $catcode_sql $special_sql $showset_sql $search_sql $coupon_sql $brand_sql $shortage_sql $stock_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc limit $start, $rows";
	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
