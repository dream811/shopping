...
//-->
      ...
      <input type="hidden" name="page" value="<?=$page?>">
        ...
        <tr>
          ...
          <td width="35%" class="t_value">
            ...
            <option value="prdname" <? if(isset($searchopt) && $searchopt == "prdname") echo "selected"; ?>>상품명
            <option value="prdcode" <? if(isset($searchopt) && $searchopt == "prdcode") echo "selected"; ?>>상품코드
            <option value="prdcom" <? if(isset($searchopt) && $searchopt == "prdcom") echo "selected"; ?>>제조사
            </select>
        ...
        <tr>
          ...
          <td class="t_value">
          	...
          	?>
          	<option value="<?=$row['idx']?>" <? if(isset($brand) && $brand == $row['idx']) echo "selected"; ?>><?=$row['brdname']?></option>
          	<?
          	}
          	?>
          </td>
      ...
      <?
			$sql = "select prdcode from wiz_product where mallid = '".$wiz_md['id']."'";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
			...
			$sql = "select distinct wp.prdcode from wiz_product wp, wiz_cprelation wc
			              where $catcode_sql $special_sql $display_sql $search_sql $coupon_sql $brand_sql $shortage_sql $stock_sql $status_sql $mallid_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc";
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr><td class="t_rd" colspan="20"></td></tr>
				...
				$sql = "select distinct wp.prdcode, wp.prdimg_R, wp.prdname, wp.sellprice, wp.prior, wp.stock, wp.mallid, wp.status from wiz_product wp, wiz_cprelation wc
				              where $catcode_sql $special_sql $display_sql $search_sql $coupon_sql $brand_sql $shortage_sql $stock_sql $status_sql $mallid_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc limit $start, $rows";
				$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
