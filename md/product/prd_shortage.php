...
//-->
      ...
      <?
			...
			$sql = "select distinct wp.prdcode from wiz_product wp, wiz_cprelation wc
			              where $catcode_sql $stock_sql $search_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc";
			//echo $sql;
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
      ...
      <?
			$sql = "select distinct wp.prdcode, wp.optcode, wp.optcode2, wp.optvalue, wp.prdimg_R, wp.prdname, wp.sellprice, wp.prior, wp.stock, wp.savestock, wp.opt_use from wiz_product wp, wiz_cprelation wc
		              where $catcode_sql $stock_sql $search_sql wc.prdcode = wp.prdcode and wp.mallid = '".$wiz_md['id']."' order by wp.prior desc, wp.prdcode desc limit $start, $rows";
			//echo $sql;
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
