...
-->
     ...
     <br>
			<?
			$sql = "select idx from wiz_basket where status != '' and (status = 'CA' or status = 'CI' or status = 'CC') and mallid = '".$wiz_md['id']."'";
			...
			$sql = "select * from wiz_basket where (status = 'CA' or status = 'CI' or status = 'CC') and mallid = '".$wiz_md['id']."' $status_sql $period_sql $searchopt_sql $reason_sql order by ca_date desc";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
