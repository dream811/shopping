...
if($mode == "chgstatus"){
	...
	//$sql = "update wiz_basket set ord_status = '$chg_status', deliver_num='$deliver_num', deliver_date='$deliver_date' where orderid = '$orderid' and mallid = '".$wiz_md['id']."'";
	$sql = "update wiz_basket set ord_status = '$chg_status' where orderid = '$orderid' and mallid = '".$wiz_md['id']."'";
	mysqli_query($connect, $sql) or die(mysqli_error($connect));
...
// 주문상태 일괄변경
	...
	while($array_selorder[$i]){
		...
		if(strcmp($old_status, "OC") && strcmp($old_status, "RC") && strcmp($old_status, "OR")) {
			...
			$sql = "update wiz_basket set ord_status = '$chg_status', deliver_num='$deliveryno[$i]', deliver_date='$deliver_date[$i]' where orderid = '$orderid' and mallid = '".$wiz_md['id']."'";
			mysqli_query($connect, $sql) or die(mysqli_error($connect));
...
// 상품 취소
	...
	if(!strcmp($orderstatus, "OR")) {
		...
		// basket 업데이트
		$sql = "update wiz_basket set status = 'CC', admin = '".$wiz_md['id']."', bank = '$bank', account = '$account',
   	...
   	complete("상품이 취소되었습니다.","order_info.php?orderid=$row['orderid']&page=$page&$param");
		...
		// basket 업데이트
		$sql = "update wiz_basket set status = 'CA', admin = '".$wiz_md['id']."', bank = '$bank', account = '$account',
...
// 장바구니 주문상태, 운송장번호, 발송일자 업데이트
	...
	$sql = "update wiz_basket set
					...
					admin = '".$wiz_md['id']."', bank = '$bank', account = '$account',acc_name = '$acc_name',
