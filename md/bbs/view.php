...
if($bbs_row['privacy'] == "Y"){
	...
	($wiz_md['id'] != "" && strpos($bbs_row['memgrp'],$wiz_md['id']) !== false) ||				// 그룹의글
	($grp_passwd > 0)																																					// 그룹비번
	){
	}else{
...
if($mybbs) $my_sql = " and (memid='".$wiz_md['id']."' or memgrp like '".$wiz_md['id'].",%' or mallid = '".$wiz_md['id']."')";
...
if($row = mysqli_fetch_array($result)) {
	...
	if($row['privacy'] == "Y"){
		...
		if(
			($mem_level == "0") ||																																		// 전체관리자
			($bbs_info['bbsadmin'] != "" && strpos($bbs_info['bbsadmin'],$wiz_md['id']) !== false)  ||	// 게시판관리자
			($row['memid'] != "" && $row['memid'] == $wiz_md['id'])																 ||	// 자신의글
			($wiz_md['id'] != "" && strpos($row['memgrp'],$wiz_md['id']) !== false)								// 그룹의글
		){
		}else{
