...
include "../../inc/md_check.inc"; // 입점업체 로그인 체크
...
if($mode == "write"){
  ...
  if(!strcmp($bbs_info[spam_check], "Y")) {
	...
	if($passwd == "") $passwd = $wiz_md[passwd];
...
////////////////////////////////////////////////////////////////////////////////
	...
	if(
		$mem_level == "0" || 																																				// 전체관리자
		($bbs_info[bbsadmin] != "" && strpos($bbs_info[bbsadmin],$wiz_md['id']) !== false)  ||		// 게시판관리자
		($bbs_row[memid] != "" && $bbs_row[memid] == $wiz_md['id']) || 													// 자신의글
	...
	if($privacy == "Y" && ($bbs_row[memid] == "" || $wiz_md['id'] == "")) $param .= "&passwd=$passwd";
...
////////////////////////////////////////////////////////////////////////////////
  ...
  if(!strcmp($bbs_info[spam_check], "Y")) {
	...
	if(empty($name)) $name = $wiz_md[name];

	$name = str_replace("\"","&quot;",$name);
	if($passwd == "") $passwd = $wiz_md[passwd];

	$sql = "insert into wiz_comment(idx,ctype,cidx,prdcode,star,id,name,content,passwd,wdate,wip)
					values('', '$ctype', '$idx', '$prdcode', '$star', '$wiz_md['id']', '$name', '$content', '$passwd', now(), '$_SERVER[REMOTE_ADDR]')";
  $result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
