...
?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <?
			...
			$sql = "select distinct(wp.prdcode) from wiz_product wp inner join wiz_cprelation as wc on wp.prdcode = wc.prdcode where wp.prdcode != '' and wp.mallid = '".$wiz_md['id']."' $group_sql $searchopt_sql $order_sql";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
			...
			$sql = "select distinct(wp.prdcode), wp.prdname, wp.prdimg_R, wp.viewcnt, wp.deimgcnt, wp.basketcnt, wp.ordercnt, wp.cancelcnt, wp.comcnt
						...
						where wp.prdcode != '' and wp.mallid = '".$wiz_md['id']."' $group_sql $searchopt_sql $order_sql limit $start, $rows";
			$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
