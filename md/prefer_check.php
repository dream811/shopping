...
$sql = "select designer_id from wiz_shopinfo  where designer_id  = '$strID' or anywiz_id = '".md5($strID)."'";
$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
$total3 = mysqli_num_rows($result);

if($strID != ""){
	if($total > 0){
		$checkmsg = "<font color=#00BCBC><b>".$strID."</b></font> 는 존재하는 추천인코드 입니다. <a class='AW-btn' onCLick='setId()'>사용하기</a>";
	} else if($total2 + $total3 > 0) {
		$checkmsg = "<font color=#00BCBC><b>".$strID."</b></font> 는 사용할 수 없는 추천인코드 입니다.";
	} else{
		$checkmsg = "<font color=#00BCBC><b>".$strID."</b></font> 는 존재하지 않는 추천인코드 입니다. ";
	}
}else{
	$checkmsg = "사용하고자 하는 추천인코드를 입력하세요";
}
?>
<html>
<head>
