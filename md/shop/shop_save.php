...
if($mode == "mall_info"){
	...
	$sql = "update tb_users set passwd='$passwd',com_name='$com_name',com_owner='$com_owner',com_num='$com_num',com_kind='$com_kind',
...
// 운영정보설정
	...
	$sql = "update tb_users set del_com='$del_info[0]', del_trace='$del_info[1]', del_prd='$del_prd', del_prd2='$del_prd2', del_method ='$del_method', del_fixprice ='$del_fixprice', del_staprice ='$del_staprice', del_staprice2 ='$del_staprice2', del_staprice3 ='$del_staprice3',
...
// 구매가이드설정
	...
	$sql = "update wiz_mall set guide = '$content' where id = '".$wiz_md['id']."'";
	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
...
// 입점업체 탈퇴
	...
	// 입점업체 상품 미승인상태로 변경
	$sql = "update wiz_product set status = 'N' where mallid = '".$wiz_md['id']."'";
	mysqli_query($connect, $sql) or die(mysqli_error($connect));

	// 장바구니에 해당 업체 상품 삭제
	$sql = "delete from wiz_basket_tmp where mallid = '".$wiz_md['id']."'";
	mysqli_query($connect, $sql) or die(mysqli_error($connect));

	// 탈퇴신청 내용 삭제(입점업체 아이디를 [out] 으로 처리)
	$sql = "insert into wiz_bbs(idx,code,memid,name,tphone,subject,content,ip,wdate)
					values('','mallout','".$wiz_md['id']."','".$wiz_md['name']."','".$wiz_md['com_tel']."','$subject','$content','$REMOTE_ADDR',unix_timestamp('".date('Y-m-d H:i:s')."'))";
	mysqli_query($connect, $sql) or die(mysqli_error($connect));
