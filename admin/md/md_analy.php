...
<? include "../header.php"; ?>
  ...
  ?>
	...
	$sql = "select * from tb_users $period_sql ";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="2">
	  <tr>
	    <td class="tit_sub"><img src="../image/ics_tit.gif"> MD등급별통계</td>
	  </tr>
	</table>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      $sql = "select idx,level,name from wiz_mdlevel  order by level asc";
      $result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
      while($row = mysqli_fetch_object($result)){
      	
      	$m_sql = "select id from tb_users where md_level = '$row->idx'";
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="2">
	  <tr>
	    <td class="tit_sub"><img src="../image/ics_tit.gif"> 스크레핑등급별 통계</td>
	  </tr>
	</table>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      $sql = "select idx,level,name from wiz_scraplevel  order by level asc";
      $result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
      while($row = mysqli_fetch_object($result)){
      	
      	$m_sql = "select id from tb_users where sc_level = '$row->idx'";
      	$m_result = mysqli_query($connect, $m_sql) or error(mysqli_error($connect));
      	$member_cnt = mysqli_num_rows($m_result);
  ...
  <br>
    ...
    <?
       $sql = "select substring(address,1,2) as address, count(address) areatotal from tb_users group by substring(address,1,2) order by areatotal";
       $result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
       $total = 0;
       while($row = mysqli_fetch_object($result)){
