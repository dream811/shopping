...
function userDelete(){
	...
	if(seluser == ""){
		...
		if(confirm("회원 삭제 시 입점업체가 등록한 모든 상품 및 데이터가 삭제며 모든 하부회원들은 최고관리자에 소속됩니다.\n\n선택한 업체를 정말 삭제하시겠습니까?")){
			document.location = "md_save.php?mode=deluser&seluser=" + seluser;
		}
	}
}
...
//-->
	  ...
	  <table border="0" cellspacing="0" cellpadding="2">
	    <tr>
	      ...
	      <td valign="bottom" class="tit">MD회원목록</td>
	      <td width="2"></td>
    ...
    <br>
    <?
		$sql = "select count(id) as all_total from tb_users";
      ...
      <tr><td class="t_rd" colspan="20"></td></tr>
	  	...
	  	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect));
			...
			while(($row = mysqli_fetch_object($result)) && $rows){
				...
				$md_sql = "select name from wiz_mdlevel where idx = '$row->md_level' limit 1";
				$md_result = mysqli_query($connect, $md_sql) or error(mysqli_error($connect));
				$md_row = mysqli_fetch_object($md_result);
				if(isset($md_row->name)) $row->md_level = $md_row->name;
				else $row->md_level = "일반";

				$sc_sql = "select name from wiz_scraplevel where idx = '$row->sc_level' limit 1";
				$sc_result = mysqli_query($connect, $sc_sql) or error(mysqli_error($connect));
				$sc_row = mysqli_fetch_object($sc_result);
				if(isset($sc_row->name)) $row->sc_level = $sc_row->name;
				else $row->sc_level = "일반";
			?>
     <form name="frm<?=$no?>" action="md_save.php" name="<?=$row->prdcode?>" method="get">
      ...
      <tr class="t_tr">
        ...
        <td align="center"><a href="md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>"><?=$no?></a></td>
        <td align="center"><a href="md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>"><?=$row->name?></a></td>
        <td align="center"><a href="md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>"><?=$row->md_level?></a></td>
        <td align="center"><a href="md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>"><?=$row->sc_level?></a></td>
        <td align="center"><a href="md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>"><?=$row->strID?></a></td>
        <td align="center"><?=$hphone?></td>
        <td align="center"><a href="javascript:sendEmail('<?=$row->com_name?>:<?=$row->email?>')"><?=$row->email?></a></td>
        <?
        $mall_sql = "select prdcode,status from wiz_product where mdid='$row->id'";
        $mall_result = mysqli_query($connect, $mall_sql) or error(mysqli_error($connect));
        $prd_total = mysqli_num_rows($mall_result);
        ?>
        <td align="center"><a href="../product/prd_list.php?s_mdid=<?=$row->id?>" target="_blank"><?=$prd_total?></a></td>
        <!-- <td align="center"><a href="/shop/minishop.php?mdid=<? //=$row->id?>" target="_blank">[방문]</a></td>
        <td align="center"><a href="./account_list.php?searchopt=wb.mdid&searchkey=<?//=$row->id?>" target="_blank">[검색]</a></td> -->
        ...
        <td align="center"><a onClick="document.location='md_info.php?mode=update&id=<?=$row->id?>&<?=$param?>&page=<?=$page?>';" class="AW-btn-s">상세보기</a></td>
      </tr>
